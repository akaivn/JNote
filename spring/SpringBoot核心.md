...
![Springboot](https://img.shields.io/badge/Springboot-v2.2.1-blue?logo=spring)
...
@**PropertySource**：加载指定的配置文件；**只能加载properties配置文件**
...
切换thymeleaf版本，老版本的springboot是默认引入2.x.x版本的thymeleaf,先在引入的都是最新版
<properties>
...
​	1、**DefaultErrorAttributes：把错误处理信息做数据共享**
...
​	3、**ErrorPageCustomizer：支持定制error错误页面，加上/error**
...
​	4、**DefaultErrorViewResolver：操作要跳转的html页面**
...
docker run --name mysqltcl -e MYSQL_ROOT_PASSWORD=kate -d mysql:5.6 --character-set-server=utf8mb4 --collation-server=utf8mb4_unicode_ci
指定mysql的一些配置参数
```

```shell
启动mysql的最终命令
docker run -p 3306:3306 --name mysqltcl -e MYSQL_ROOT_PASSWORD=kate -d mysql:5.6 --character-set-server=utf8mb4 --collation-server=utf8mb4_unicode_ci
```

## 5、Docker对Redis(5.x)配置
...
```shell
# 容器启动命令
docker run -d -p 6379:6379 --name redistcl 
-v /docker/redis/conf/redis.conf:/usr/local/etc/redis/redis.conf
-v /docker/redis/data:/data redis:5.0 redis-server /usr/local/etc/redis/redis.con
--appendonly yes
#客户端启动命令
...
# -p：指定端口映射
# --name：给容器起别名
# -v /docker/redis/conf/redis.conf(本地的配置文件位置):/usr/local/etc/redis/redis.conf(docker redis配置文件位置) 把本地的配置文件挂载到docker redis镜像中
# -v /docker/redis/data(本机位置):/data(docker redis数据位置) 把docker redis上产生的数据备份到本地，因为docker容器中redis持久化后产生的数据会随着docker容器的消失而消失，所以我们选择数据备份同步到主机，使用数据卷
# redis:5.0：指定redis镜像
# redis-server /usr/local/etc/redis/redis.conf 指定redis启动时和指定的配置文件一起启动
# --appendonly yes 开启redis数据持久化
```

​	**注意：配置数据卷的过程中，如果一直开启后闪退, 就把redis配置文件的daemonize yes 改为no ,默认no**
...
#注意：到tomcat8以后，通过浏览器访问默认为8080页面，因为tomcat的文件夹的webapps文件夹里没有任何文件，它默认的测试文件在webapps.dist下

# 添加SSL证书后的启动命令，详细证书配置参照各云厂商文档
$ docker run -d --name tomcattcl -p 8080:8080 -v /data/tomcat/webapps:/usr/local/tomcat/webapps -v /data/tomcat/conf/server.xml:/usr/local/tomcat/conf/server.xml -v /data/tomcat/conf/web.xml:/usr/local/tomcat/conf/web.xml -v /data/tomcat/conf/riskate.club.jks:/usr/local/tomcat/conf/riskate.club.jks tomcat:9.0
...
mysql驱动到8.x之后，他的驱动类的全类名可以换成com.mysql.cj.jdbc.Driver，当然以前的也可以用，它默认向下兼容
```
...
​	2.x之前默认是用**org.apache.tomcat.jdbc.pool.DataSource**作为数据源；

​	2.x之后默认是用**class com.zaxxer.hikari.HikariDataSource**作为数据源;
